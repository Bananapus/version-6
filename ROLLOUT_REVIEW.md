...
- [x] Buyback 1.4.1 and core 1.2.1 are published; their earlier npm permission failures are superseded. Router 1.3.1 is published; 1.3.2 is prepared in router PR #158.

Successful publication evidence: [buyback 1.4.1](https://github.com/Bananapus/nana-buyback-hook-v6/actions/runs/34648683573), [core 1.2.1](https://github.com/Bananapus/nana-core-v6/actions/runs/34650160259). Core #210 and buyback #179 have been merged by the maintainer. Router #158 is fully green after its historical-artifact test fix. Deploy-all #246 remains open while the final verifier correction completes CI; formatting and affected local fork tests pass.
...
- Deploy-all: the Base fork at block 51,005,824 now uses a fixed pre-rollout fixture, while newly deployed contracts are checked against production artifacts. The full suite passed 105 suites/441 tests on `c9bdea3`. Follow-up `0e2743c` makes the read-only verifier check the immediately outgoing `_deprecated1` hook/router, with `_deprecated` fallback for earlier layouts. Both false-success regressions failed before the correction and pass afterward; all eight affected fork tests pass. The corrected read-only verifier also passed on Ethereum, Optimism, Base, and Arbitrum (28 checks passed, zero failed, one optional project-operator check skipped per chain); no transactions were sent. [Final CI](https://github.com/Bananapus/deploy-all-v6/actions/runs/34651901118) is running.
