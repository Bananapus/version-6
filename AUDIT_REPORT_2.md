...
7. Package PRs are open/pushed with package versions/dependencies already bumped on the branch where applicable, and
   the deploy-all package records any cross-repo dependency coherence requirements.
...
| Map modules/dependencies/interactions | Module-to-invariant manifest and repo evidence snapshot map each runtime repo to reviewed module groups, invariants, and trust boundaries. `AUDIT_SRC_MANIFEST.md` now lists all 295 in-scope production `src/*.sol` files and ties each file to the relevant evidence bucket. Rechecked on 2026-05-21 by comparing live `src/*.sol` paths across the 18 in-scope runtime repos against manifest entries: 295 live, 295 listed, 0 missing, 0 stale. | Partially satisfied: per-file coverage exists, but not a per-function formal dependency graph. |
...
ledger, hooks/plugins, cross-chain/Revnet, apps, and deployment layers; records current evidence status for each; and
defines completion criteria for the broad formal objective. The section below is the report summary of that plan.
...
- Refreshed CI after the final proof commits. `nana-univ4-router-v6` #109, `nana-omnichain-deployers-v6` #110, and
  ...
  reports no checks.
- Added `AUDIT_FORMAL_SPEC.md`, a standalone proof-obligation plan that assigns invariant IDs across the core ledger,
