...
| HOOK-ROUTER-01 | Router terminal and buyback hooks enforce minimum-return semantics and do not silently disable slippage protection. | Router/buyback fork suites and `JBSwapLibHalmos` targets. Status: `CI-proof` for helper branches, `Invariant/Fork` for routing. | Unified router/buyback/Uniswap path invariant with adversarial route outputs and terminal fallback. |
