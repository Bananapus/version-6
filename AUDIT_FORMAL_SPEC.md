...
| --- | --- | --- | --- |
| CORE-LEDGER-01 | For every terminal, project, and token, internal terminal-store claims must not exceed actual terminal backing except for explicitly modeled held-fee, bridge-bound, or pending-transfer states. | CORE-01 regressions, callback-token guard tests, core full suite, terminal/store invariants now covering two terminals plus migration, `CoreHookCompositionInvariant` for split-hook callback re-entry, and `TestDataHookFuzzing` real data-hook pay/cash-out composition. Status: `Invariant/Fork`. | Composed model over `JBMultiTerminal`, `JBTerminalStore`, token transfer callbacks, fee handling, and migrations. |
| CORE-LEDGER-02 | Accepted funds are consumed exactly once by `pay`, `addToBalanceOf`, payout, surplus allowance, cash-out, and migration flows. | Reentrancy regressions, split-hook reentry tests, migration tests, terminal-store invariant migration action, `CoreHookCompositionInvariant` exercising payout splits that re-enter `pay`, and `TestDataHookFuzzing` proving a real data hook can drive real pay/cash-out hook forwards with matching pre/post contexts. Status: `Invariant/Fork`. | Stateful solvency invariant with terminal entrypoints, split hooks, data hooks, and migrations in one handler. |
...
| CORE-LEDGER-05 | Split locks are unique enough to avoid duplicate group/accounting ambiguity, and every split execution conserves assigned value. | CORE-06/07 tests, split conservation regressions, and `CoreHookCompositionInvariant` for terminal payout splits with real split-hook callbacks. Status: `Invariant/Fork`. | Cross-component split conservation invariant covering terminal payouts, reserved-token splits, 721 splits, distributor splits, and hook callbacks. |
