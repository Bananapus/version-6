...
- Runtime dependencies are EXACTLY: `next`, `react`/`react-dom`, `stripe`, `viem`, `pg`, `resend`. Adding any other runtime dependency requires explicit user approval first.
...
**Interfaces:**
- Produces: `JBProcessorEscrow` with `processPayment(bytes32 paymentId, IJBTerminal terminal, uint256 projectId, uint256 usdcAmount, uint256 minReturnedTokens, address projectToken, uint48 unlockAt, string memo) returns (uint256 tokensHeld)`, `release(bytes32 paymentId, address to)`, `forfeit(bytes32 paymentId)`, `entries(bytes32) -> (address token, uint160 amount, uint48 unlockAt, bool settled)`, `setOperator(address)` (owner). Events `Processed`, `Released`, `Forfeited`.
...
contract JBProcessorEscrowTest is Test {
    function test_processPayment_pullsUsdcPaysAndRecordsEntry() public { /* asserts entry fields + Processed event + tokens held by escrow */ }
    ...
    function test_release_transfersAfterUnlockAndSingleUse() public {}
...
contract JBProcessorEscrow is Ownable {
    ...
    struct Entry { address token; uint160 amount; uint48 unlockAt; bool settled; }

    error NotOperator(); error EntryExists(); error NoEntry();
    error StillLocked(); error AlreadySettled(); error UnlockPassed();

    event Processed(bytes32 indexed paymentId, uint256 indexed projectId, uint256 amountPaid, uint256 tokensHeld, uint48 unlockAt);
    ...
    function processPayment(
        ...
        entries[paymentId] = Entry(projectToken, uint160(tokensHeld), unlockAt, false);
        emit Processed(paymentId, projectId, usdcAmount, tokensHeld, unlockAt);
    }

    function release(bytes32 paymentId, address to) external onlyOperator {
        ...
        entry.settled = true;
        IERC20(entry.token).safeTransfer(to, entry.amount);
        emit Released(paymentId, to, entry.amount);
    }
...
- Produces: `processPayment(clients, {paymentId, terminal, projectId, usdcAmountWei, minReturnedTokens, projectToken, unlockAt, memo}): Promise<{txHash, tokensHeld: bigint}>` — simulates first (`simulateContract`), sends, waits one confirmation, parses the `Processed` event for `tokensHeld`; `release(clients, {paymentId, to}): Promise<{txHash}>`; `paymentIdBytes32(uuid: string): 0x-hex` (uuid → 16 bytes left-padded); `feePaymentId(paymentId)` (keccak per contract convention).
...
- Produces:
  - `createCheckoutSession(deps, {projectId, amountUsdCents, email, instant}): Promise<{url, paymentId}>` — rejects if project missing/suspended; rejects card-path amounts over `CARD_CEILING_USD_CENTS` unless method resolves to bank; rejects `instant` when pool headroom (USDC allowance minus in-flight instant payments) is insufficient; quotes tokens and stores `quote_tokens`; creates a Stripe Checkout Session (`payment_method_options.card.request_three_d_secure: 'any'`, `payment_method_types` per amount: `['card','us_bank_account']` under ceiling, `['us_bank_account']` above, line item = donation + (instant ? premium `PREMIUM_BPS` : 0), `metadata.payment_id`), stores `stripe_session_id`.
...
- Produces: `handlePay(deps, job)`:
  ...
  5. `escrow.processPayment(...)` with `minReturnedTokens = quoteNow * (10000 - DRIFT_TOLERANCE_BPS) / 10000`, `unlockAt` from the row, memo = payment id.
  6. If instant: `escrow.processPayment` again for the premium into `PROCESSOR_PROJECT_ID` (skip silently if env unset — pre-revnet, premium accrues in the settlement wallet).
...
### Task 8: Magic-link auth + account + claim + release

**Files:**
- Create: `src/auth/magic.ts`, `app/api/login/route.ts`, `app/api/claim/route.ts`, `app/api/payments/[id]/route.ts`, `app/account/page.tsx`, `app/login/page.tsx`, `app/done/page.tsx`, `src/worker/release.ts`
- Test: `test/magic.test.ts`, `test/release.test.ts`

**Interfaces:**
- Consumes: `transition`, `enqueue`, `escrow.release`, `sendEmail`.
...
- `GET /api/payments/:id` → public, returns `{state, amountUsdCents, quoteTokens, tokensHeld, unlockAt, payTx}` — the integrator surface for Artizen.
- `POST /api/claim {paymentId, address}` — session email must match payment email; address checksummed-validated (viem `isAddress`); stores `claim_address`; if state is `unlocked`, enqueues `release` (dedupe `release:<id>`).
- `handleRelease(deps, job)` — requires `unlocked` + `claim_address`; `escrow.release`; `transition(['unlocked'],'claimed',{release_tx})`; confirmation email. `unlock-note` job handler: `transition(['held'],'unlocked')` + "your tokens are ready" email (both token legs: project + $PROCESSOR fee entry noted when present).
- Account page (RSC): payments for session email — project name, amount, state label, expected/held tokens, unlock countdown (server-computed days), claim form when `unlocked`, BaseScan links for `pay_tx`/`release_tx`. No wallet library — claim is paste-an-address (viem `isAddress` check server-side); connect-wallet autofill can come later.

- [ ] **Step 1: Failing tests** — token round-trip, expiry, single-use, tamper; claim rejects mismatched session email and bad address; release handler transitions and calls escrow with stored address; unlock-note flips `held→unlocked`.
...
- [ ] **Step 3: `vitest` → PASS; `next build` passes (the real gate). Commit** `feat: magic-link accounts, claim flow, release worker`.
...
- **Spec coverage:** checkout/3DS/ceiling (T6, T10), settlement-triggered default + premium T0 + pool allowance (T6, T7), escrow contract + hold + forfeit (T1, T5, T7), claim + account page + magic link (T8), eligibility application gate (manual SQL insert + checklist in runbook, T10) + auto-suspend watcher (T9), reconciliation + alerts (T9), refund-on-drift (T7), dispute handling both windows (T6), $PROCESSOR fee leg with pre-revnet accrual fallback (T7), Base-only + env tunables (global constraints). Deliberately deferred per spec: recovery cash-out is a manual runbook step; premium availability = allowance headroom check; no admin UI.
- **Type consistency:** `transition(pool, id, from[], to, patch)` used identically in T6–T9; `processPayment` TS signature mirrors the Solidity ABI in T1; `paymentIdBytes32`/`feePaymentId` defined in T5, consumed in T7.
