...
- [ ] Homerun: feat/preview-create built (7 tasks), reviewed (one fix round + held-record persistence for a confirmed relay launch Center has not recorded), PR opened and merging; Railway deploy watched; live check of the new flow on dev running (harness at homerun/.superpowers/live/preview/homerun-preview-live.mjs, copy in scratchpad).
- [x] Center follow-up: jbcenter PR #54 re-queues a named failed chain that paid for no bundle (mirrored #55); dev caps raised (SPONSOR_MAX_FEE_PER_GAS 30 gwei, daily budget 5 ETH) so Sepolia fits; Sepolia retry on intent 59442f3d running.
