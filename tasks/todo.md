...
- [x] Homerun correction: PR #4 (feat/preview-full-page) renders the preview and the intent link as the full project page, Create is one press with the wallet chooser inline, the pre-attach title is kept, the IPFS gateway follows the configured Center; screenshot checked; merging and deploying.
