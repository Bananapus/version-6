...
- [ ] Homerun correction in progress (feat/preview-full-page): jango expects the preview and the intent link to render the full /founderhaus-style project page, not the bare shell; also the IPFS gateway must follow the configured Center (dev pins lag behind production's gateway).
