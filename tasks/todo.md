...
- [x] 2026-09-23: every create text field renders on the live, preview and intent pages (homerun PR #9): revenue description, minimum revenue and consequences in the Income stage, profiles' introductions, location, FUND token block on Owners. Follow-ups fixed in PR #10 (one FundTokenTerms block on Owners for all three pages, Splits heading rhythm, one money formatter). The Splits chart now shows the planned 70/10/20 with "Planned allocation until INCOME starts." before any INCOME exists (PR #11). Minor open: in the income stage two charts share id rev-payment-split.
