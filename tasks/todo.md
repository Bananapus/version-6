...
- [ ] Center: implementer was mid-run in extensions/center-setup-calls (branch feat/intent-relay) when jango paused; on resume check `git log c803010..HEAD` there and .superpowers/sdd/relay/report.md, then resume or re-dispatch. Ruling added: verifier records `forwarded` per deployment (new migration), API returns it, /deploy and /relay refuse 409 mixed_sender when any deployment is not forwarded; rebase onto main (PR #50 binding fix) before the gate.
...
- [ ] Homerun: not started (waits for nothing now: 2.10.0 is on npm); implementer prompt = plan docs/superpowers/plans/2026-09-22-homerun-preview-create.md in extensions/homerun-setup-calls (branch feat/preview-create from 2dd22bc); its live check needs Center's relay route deployed.
