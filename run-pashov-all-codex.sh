#!/bin/bash
# Pashov Solidity Auditor (Codex) — automated run across all JB V6 repos
# Usage: ./run-pashov-all-codex.sh [repo-name ...]
# Logs: .audit-logs/<repo>-codex-pashov-<timestamp>.log
# Monitor: tail -f .audit-logs/codex-pashov-summary-*.log
# Reports: .audit/findings/<repo>-codex-pashov-ai-audit-report-<timestamp>.md
...
# All repos in ecosystem, kept close to the Nemesis ordering.
REPOS=(
  "nana-core-v6"
...
SUMMARY="$LOG_DIR/codex-pashov-summary-$TIMESTAMP.log"

echo "=== Pashov Solidity Auditor (Codex) Run: $TIMESTAMP ===" | tee "$SUMMARY"
...
PROMPT_DIR="/tmp/codex-pashov-prompts-$TIMESTAMP"
mkdir -p "$PROMPT_DIR"
...
# STEP 0: Build repo-specific context bundles.
for REPO in "${REPOS[@]}"; do
...
# STEP 1: Clear prior Codex Pashov report files.
PREV_DIR="$BASE/.audit-findings-pashov-prev-$TIMESTAMP"
mkdir -p "$PREV_DIR"
HIDDEN_COUNT=0
...
for REPO in "${REPOS[@]}"; do
  ...
  if [ -d "$REPORTS_DIR" ]; then
    REPORT_FILES=$(find "$REPORTS_DIR" -maxdepth 1 -name "*-codex-pashov-ai-audit-report-*.md" 2>/dev/null)
    if [ -n "$REPORT_FILES" ]; then
      mkdir -p "$PREV_DIR/$REPO"
      cp -a "$REPORTS_DIR"/*-codex-pashov-ai-audit-report-*.md "$PREV_DIR/$REPO/" 2>/dev/null || true
      rm -f "$REPORTS_DIR"/*-codex-pashov-ai-audit-report-*.md
      HIDDEN_COUNT=$((HIDDEN_COUNT + 1))
    fi
  fi
done

echo "Cleared prior Codex Pashov reports from $HIDDEN_COUNT repos → $PREV_DIR" | tee -a "$SUMMARY"
echo "" | tee -a "$SUMMARY"

# STEP 2: Run the Solidity Auditor via Codex.
for i in "${!REPOS[@]}"; do
  REPO="${REPOS[$i]}"
  REPO_DIR="$BASE/$REPO"
  LOG_FILE="$LOG_DIR/${REPO}-codex-pashov-${TIMESTAMP}.log"
  ...
  CODEX_PROMPT="You are running the local Pashov Audit Group 'solidity-auditor' skill for this repository.
...
- This is a non-interactive Codex CLI run. Do not rely on slash commands or editor-specific tools.
...
- Write the markdown report to .audit/findings/{project-name}-codex-pashov-ai-audit-report-{timestamp}.md in this repo.
...
=== SOLIDITY AUDITOR SKILL AND REFERENCES ===

$(cat "$STATIC_SKILL_BUNDLE")
...
$(cat "$CONTEXT_BUNDLE")"
  ...
  if [ -d "$REPO_DIR/.audit/findings" ]; then
    REPORT_COUNT=$(find "$REPO_DIR/.audit/findings" -maxdepth 1 -name "*-codex-pashov-ai-audit-report-*.md" | wc -l | tr -d ' ')
    echo "  Pashov reports: $REPORT_COUNT files" | tee -a "$SUMMARY"
  else
...
echo "Previous Pashov reports backed up to: $PREV_DIR" | tee -a "$SUMMARY"
