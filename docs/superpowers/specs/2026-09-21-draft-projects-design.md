# Draft projects: create without a transaction
...
A draft is a Center intent, unchanged in shape:
...
- `deploymentCalls`: one `{chainId, to, data}` per chain, frozen calldata targeting the same
  ...
  (the time the draft was made, or the time the creator chose) rather than 0, so a late
...
- `jb`: the publishing client's own form data (`CreateDraft` for juicebox.money, the revnet
  draft for revnet.money, and so on) so the home client can re-open the draft in its wizard.
...
- Supersede: a new intent whose envelope names `supersedes: <intentId>`, signed by the same
  publisher. The old intent leaves search, and its URL redirects to the new one.
- Withdraw: a signed message `Juice Central withdraw intent\nVersion: 1\nIntent: <id>` from
  the publisher. Withdrawn intents leave search and their URL shows "withdrawn".
...
Exactly one party deploys all of a draft's chains, so every chain sees the same sender.
...
A self-paid and a sponsored deploy racing on one draft inside the confirmation window can
...
- `mergeSearch(bendystrawRows, centerItems)`: one list ordered by creation time, drafts
  flagged `draft: true`.
...
- `draftPath(id)` = `/draft/<id>` and `deployedRedirect(intent)` for the route convention.
- Intent client: `supersede`, `withdraw`, `requestDeploy`, deploy status on `getIntent`.
- A `jb-draft-projects` skill in juicebox-skills states the norm for generated UIs.
...
local draft, land on `/draft/<id>`. "Deploy now" stays as the self-paid path and is the only
path when the draft includes Ethereum mainnet.

**Lists and search.** Wherever a client lists or searches projects it calls Center search
next to Bendystraw and merges. Trending and Top stay volume-based, so drafts never rank
there. Each draft row shows a draft badge.

**Project page.** `/draft/<id>` renders the client's normal project page from the decoded
shell, reusing the existing degraded-shell path where one exists (`shellProject` in
juicebox.money, `getProjectFallback` in revnet.money). A draft badge and a "Deploy" action
replace the stats that need chain data. Once Center reports any deployment the route
redirects to the deployed URN. Superseded drafts redirect to their successor; withdrawn
ones show a notice.
...
JBSticky. When the target project is a draft, the chokepoint inserts `ensureDeployed` as
...
4. eth.shop, ethis.money and JBChat: render drafts wherever they show projects.
...
- Clients: the existing vitest suites gain draft cases for the create step, the search
  merge, the draft page and the write pre-step. A browser pass on dev: publish a draft
  from a passkey account, find it in search on the other client, pay it from a second
  wallet, and land on the deployed page.

## Out of scope

Drafts proposed for owners other than the publisher as a distinct feature, draft
comments or reviews, Bendystraw indexing of drafts, and any on-chain helper contract.
